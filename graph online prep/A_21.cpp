    ...
    while(!q_cells.empty()) {
        ...
        for(auto [dx, dy] : moves) {
            ...
            if(nx>=0 && nx<n && ny>=0 && ny<m) {
                if(dist[nx][ny]==-1 && dist[x][y]+1 <= r.h) {
...
int main() {
    ...
    riders.resize(q);
    for(int i=0;i<q;i++) {
        cin >> riders[i].x >> riders[i].y >> riders[i].k >> riders[i].h;
    }
    
    // For each rider, compute distances to all cells
